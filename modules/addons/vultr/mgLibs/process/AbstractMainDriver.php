...
use MGModule\vultr\Configuration;
...
abstract class AbstractMainDriver
{
     ...
     * @param mixed $input
     */
    public static function vultrDump($input)
    {
        if (self::I()->isDebug()) {
            echo "<pre>";
            print_r($input);
    ...
    /**
     *
     * @return \MGModule\vultr\Configuration
     */
    ...
    public function setMainLangContext()
    {
     ...
     * @throws main\mgLibs\exceptions\System
     */
    public function runController(string $controller, $action = 'index', $input = array(), $type = 'HTML')
    {
        try {
            ...
            switch ($type) {
                case 'HTML':
                    ...
                    $success = $result['vars']['success'] ?? false;
                    $error = $result['vars']['error'] ?? false;
